...
import { MantineProvider, Grid, Title, Paper, Text, GridCol, Image, Badge, Accordion, AccordionControl, AccordionPanel } from "@mantine/core";
...
function WeatherDetails() {
          ...
          <Accordion.Item key={item.dt} value={item.dt.toString()}>
            ...
            <AccordionPanel>
              <Grid>
                ...
                <GridCol span={6}>
                  <WaterDropIcon style={{ fontSize: 30, verticalAlign: "middle" }} />Dew Point: {Math.round(item.dew_point)}ºC
                </GridCol>
                <GridCol span={6}>
                  {item.summary}
                </GridCol>
  ...
  if (!weatherData) { //only render the page if weatherData isnt null
    return (
      <></>
    )
  }
            ...
            <Paper shadow="xl" withBorder p={2} style={{ minHeight: 150 }}>
              ...
              <Grid justify='center' align='center' style={{ width: '100%' }}>
                ...
                <GridCol span={6} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                  <Image src={`https://openweathermap.org/img/wn/${weatherData.current.weather[0].icon}.png`} style={{ width: 50 }} />
          ...
          <Grid.Col span={4}>
            <Paper shadow="xl" withBorder p={10} style={{ minHeight: 150 }}>
              <Grid>
                ...
                <GridCol span={6}>
                  <WaterDropIcon style={{ fontSize: 30, verticalAlign: "middle" }} />Dew Point: {Math.round(weatherData.current.dew_point)}ºC
                </GridCol>
              </Grid>
            </Paper>
          </Grid.Col>
        ...
        <Grid>
          <Grid.Col span={6}>
            <Paper shadow="xl" withBorder p={10}>
              ...
              <LineChart data={weatherData.hourly} dataKey="dt" series={[{ name: "temp", color: "blue.6" }]} h={350} />
            </Paper>
          </Grid.Col>
          <Grid.Col span={6}>
            <Paper shadow="xl" withBorder p={10}>
